...
class DetectionTransformer(BaseDetector, metaclass=ABCMeta):
    ...
    def predict(self,
        ...
        """
        img_feats = self.extract_feat(batch_inputs) # [1,3,800,1199] -> [(1,2048,25,38)] -> img_feats[(1,256,25,38)]
        head_inputs_dict = self.forward_transformer(img_feats,
                                                    batch_data_samples)
    ...
    def _forward(
        ...
        img_feats = self.extract_feat(batch_inputs)
        head_inputs_dict = self.forward_transformer(img_feats,
                                                    batch_data_samples) # 取到这里 hidden_state{[layers,1,num_queries,dim]}
    ...
    def forward_transformer(self,
        ...
        encoder_inputs_dict, decoder_inputs_dict = self.pre_transformer(
            img_feats, batch_data_samples)  # img_feats: tuple([1,256,25,38]) 
    ...
    def extract_feat(self, batch_inputs: Tensor) -> Tuple[Tensor]:
        ...
        """
        x = self.backbone(batch_inputs) # batch_inputs[1,3,800,1199] -> x:tuple([1,2048,25,38])
        if self.with_neck:
            x = self.neck(x)    #  x:tuple([1,256,25,38])
        return x
