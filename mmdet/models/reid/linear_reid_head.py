...
class LinearReIDHead(BaseModule):
    ...
    def forward(self, feats: Tuple[torch.Tensor]) -> torch.Tensor:
        ...
        feats = feats[-1]       # feats[1,256] origin:[128,2048]

        for m in self.fcs:
            feats = m(feats)
        feats = self.fc_out(feats)  #[128,128]
        return feats    # feats[30,128] 这里是 fc out_channel，还没用 num_cls
    ...
    def loss(self, feats: Tuple[torch.Tensor],
        ...
        feats = self(feats)     # feats[batch,2048] -> [32,128] 将图像 下采样(1/32)->GAP(2048)->proj(128) 特征向量
    ...
    def loss_by_feat(self, feats: torch.Tensor,
        ...
        losses = dict() # feats[32,128]
        gt_label = torch.cat([i.gt_label.label for i in data_samples])
        gt_label = gt_label.to(feats.device)    # gt_label[32] 对应分类id
        ...
        if self.loss_cls:
            ...
            cls_score = self.classifier(feats_bn)   # cls_score [32,380]
            losses['ce_loss'] = self.loss_cls(cls_score, gt_label)  # cls_score[32,380],gt_label[32]
