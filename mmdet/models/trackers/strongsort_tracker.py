    ...
    def track(self,
        ...
        if self.with_reid:
            if self.reid.get('img_norm_cfg', False):
                ...
                reid_img = imrenormalize(img, img_norm_cfg, #img[1,3,800,1440] img_norm_cfg[mea,std]
                                         self.reid['img_norm_cfg']) # reid_img[1,3,800,1440]
            else:
                reid_img = img.clone()
        ...
        if self.empty or bboxes.size(0) == 0:
            ...
            self.num_tracks += num_new_tracks
            if self.with_reid:
                crops = self.crop_imgs(reid_img, metainfo, bboxes.clone(),  #boxes[30,4]
                                       rescale) #crops[30,3,256,128]
                if crops.size(0) > 0:
            ...
            if self.with_reid:
                crops = self.crop_imgs(reid_img, metainfo, bboxes.clone(),
                                       rescale)
                embeds = model.reid(crops, mode='tensor')   # embeds[30,128]
